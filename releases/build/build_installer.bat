...
cd "%BUILD_DIR%"
"%VENV_PYTHON%" -m PyInstaller -F -w --clean --uac-admin --collect-all openai --collect-submodules httpx --collect-submodules anyio --distpath "%PROJECT_ROOT%\releases\latest" --workpath "%BUILD_DIR%\build" --specpath "%BUILD_DIR%" --name installer --icon "%PROJECT_ROOT%\icon.ico" --add-data "%PROJECT_ROOT%\SubtitleTranslate - ChatGPT.as;." --add-data "%PROJECT_ROOT%\SubtitleTranslate - ChatGPT.ico;." --add-data "%PROJECT_ROOT%\SubtitleTranslate - ChatGPT - Without Context.as;." --add-data "%PROJECT_ROOT%\SubtitleTranslate - ChatGPT - Without Context.ico;." --add-data "%BUILD_DIR%\language_strings.json;." --add-data "%BUILD_DIR%\model_token_limits.json;." --add-data "%PROJECT_ROOT%\LICENSE;." "%PREPARED_INSTALLER_SCRIPT%"
if %errorlevel% neq 0 (
...
del /f /q "%BUILD_DIR%\installer.spec"
